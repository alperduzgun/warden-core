...
class PythonOrphanDetector(AbstractOrphanDetector):
    ...
    def detect_unused_imports(self) -> List[OrphanFinding]:
        ...
        for node in ast.walk(self.tree):
            if isinstance(node, ast.Import):
                ...
                if node.lineno in type_checking_lines:
                    continue
                # Skip `from __future__ import ...` — PEP directives (e.g.
                # annotations, generator_stop) that are always "used" implicitly
                # and never appear as Name references (#636).
    ...
    def detect_unreferenced_definitions(self) -> List[OrphanFinding]:
        ...
        for def_name, (line_num, def_type, def_code, node) in definitions.items():
            if def_name not in references:
                ...
                # These are referenced by type in other files, not called directly here.
                if def_type == "class" and self._is_port_or_interface(def_name, node):
                    continue

                # Skip FastAPI dependency-injection functions (#637).
                # Depends() functions are called by the framework, not directly in code.
    ...
    def _is_port_or_interface(self, name: str, node: ast.AST) -> bool:
        ...
        These classes exist to define contracts for dependency injection and are
        only referenced by type annotations in other files.  Single-file analysis
        cannot see those cross-file usages, so they would always appear as orphans.

        Heuristics (any one is sufficient):
        - File path contains ports/, interfaces/, abstractions/, or repositories/ dir
        - Class name ends with Port, Interface, Repository, Gateway, Service, or ABC
    ...
    def _is_fastapi_dependency(self, name: str) -> bool:
        ...
        FastAPI Depends() functions are called by the framework via type
        annotations, not by direct reference in the same file.  They always
        start with `get_` and live in dependency/DI modules (#637).

        Heuristics (any one is sufficient):
        - File name is dependencies.py, deps.py, di.py, or inject.py
        - File path contains dependencies/ or deps/ directory
        - Function name starts with `get_` (common FastAPI convention)
        """
        import os as _os
        basename = _os.path.basename(self.file_path)
        _DI_FILENAMES = {"dependencies.py", "deps.py", "di.py", "inject.py",
                         "dependencies.py", "providers.py"}
        if basename in _DI_FILENAMES:
            return True
        parts = set(_os.path.normpath(self.file_path).split(_os.sep))
        if parts & {"dependencies", "deps", "di"}:
            return True
        # Also skip get_* functions — overwhelmingly FastAPI DI by convention
