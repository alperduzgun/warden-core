...
class TestPhase1Security:
    ...
    def test_exception_cleanup(self):
        ...
        # Cleanup lives on orchestrator, state consistency on post-processor
        assert hasattr(PhaseOrchestrator, "_cleanup_on_completion_async")
        assert hasattr(FindingsPostProcessor, "ensure_state_consistency")
...
class TestPhase3HighPriority:
    ...
    def test_rate_limiter(self):
        ...
        limiter = TokenBucketLimiter(tpm=60, burst=10)
        assert limiter.tokens == 10

    def test_token_counter(self):
        """ID 22: Token counting with tiktoken."""
        from warden.shared.utils.token_utils import TokenCounter
    ...
    def test_pii_masking(self):
        ...
        masked = PIIMaskingFilter.mask_message("api_key=sk-1234567890abcdefghij")
        assert "***" in masked
