...
class TestApplyBaseline:
    ...
    def test_filters_known_issues(self, tmp_path):
        ...
        # Baseline fingerprint = sha256(rule_id:file_path:message).
        # The message must match the finding's message for the fingerprint to align.
        baseline = {
            "frame_results": [
                {
                    "findings": [
                        {"rule_id": "SEC-001", "file_path": "app.py", "message": "test finding"},
                    ]
                }
            ]
        }
    ...
    def test_failed_frame_becomes_passed(self, tmp_path):
        ...
        baseline = {
            "frame_results": [
                {
                    "findings": [
                        {"rule_id": "R1", "file_path": "app.py", "message": "test finding"},
                    ]
                }
            ]
        }
    ...
    def test_partial_match(self, tmp_path):
        ...
        # _compute_finding_fingerprint uses finding["id"] (or "rule_id") + file_path + message.
        # Use the same id value in both baseline and findings so fingerprints align.
        baseline = {
            "frame_results": [
                {
                    "findings": [
                        {"rule_id": "R1", "file_path": "app.py", "message": "test finding"},
                        {"rule_id": "R2", "file_path": "app.py", "message": "test finding"},
                    ]
                }
            ]
        }
        ...
        ctx = make_context(project_root=tmp_path)
        f1 = make_finding("R1")  # id="R1" matches baseline rule_id="R1"
        f1.file_path = "app.py"
        f2 = make_finding("R2")  # id="R2" matches baseline rule_id="R2"
        f2.file_path = "app.py"
        f3 = make_finding("R3")  # id="R3" not in baseline — survives
        f3.file_path = "app.py"
    ...
    def test_relative_path_normalization(self, tmp_path):
        """Baseline with relative path matches relative finding path."""
        # Fingerprint is sha256(rule_id:file_path:message); both sides must
        # use the same path representation.  Baseline generated by warden
        # stores relative paths, so relative-relative always matches.
        baseline = {
            "frame_results": [
                {
                    "findings": [
                        {"rule_id": "R1", "file_path": "src/app.py", "message": "test finding"},
                    ]
                }
            ]
        }
