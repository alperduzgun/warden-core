...
class LlmOperationsMixin:
    ...
    async def AnalyzeWithLlm(self, request, context) -> "warden_pb2.LlmAnalyzeResult":
        ...
        try:
            ...
            async for chunk in self.bridge.analyze_with_llm_async(
                code=request.code, prompt=request.prompt, provider=request.provider or None
            ):
            ...
            return warden_pb2.LlmAnalyzeResult(
                success=True, response="".join(chunks), provider_used=provider_used, duration_ms=duration_ms
            )

        except Exception as e:
            logger.error("grpc_llm_error: %s", str(e))
            return warden_pb2.LlmAnalyzeResult(
                success=False, error=warden_pb2.LlmError(code="LLM_ERROR", message=str(e))
            )
    ...
    async def ClassifyCode(self, request, context) -> "warden_pb2.ClassifyResult":
        ...
        try:
            result = await self.bridge.classify_code(code=request.code, file_path=request.file_path or None)
        ...
        try:
            ...
            if hasattr(self.bridge, "test_provider"):
                result = await self.bridge.test_provider(
                    provider=request.provider, prompt=request.prompt or "Hello, respond with OK"
                )
            ...
            async for chunk in self.bridge.analyze_with_llm_async(
                code="print('test')", prompt="Say OK", provider=request.provider
            ):
            ...
            return warden_pb2.TestProviderResponse(
                success=False, provider=request.provider, error_message="No response from provider"
            )

        except Exception as e:
            logger.error("grpc_test_provider_error: %s", str(e))
            return warden_pb2.TestProviderResponse(success=False, provider=request.provider, error_message=str(e))

    async def GetAvailableModels(self, request, context) -> "warden_pb2.AvailableModelsResponse":
        ...
        try:
            ...
            if request.provider == "anthropic":
                ...
                response.models.append(
                    warden_pb2.LlmModel(
                        id="gpt-4-turbo", name="GPT-4 Turbo", provider="openai", max_tokens=4096, available=True
                    )
                )
    ...
    async def ValidateLlmConfig(self, request, context) -> "warden_pb2.ValidateLlmConfigResponse":
        ...
        try:
            ...
            if request.provider == "anthropic":
                if "api_key" not in config and "ANTHROPIC_API_KEY" not in config:
                    warnings.append("API key not found in config (may be set via environment)")
            elif request.provider == "openai":
                if "api_key" not in config and "OPENAI_API_KEY" not in config:
                    warnings.append("API key not found in config (may be set via environment)")

            return warden_pb2.ValidateLlmConfigResponse(valid=len(errors) == 0, errors=errors, warnings=warnings)

        except Exception as e:
            logger.error("grpc_validate_config_error: %s", str(e))
            return warden_pb2.ValidateLlmConfigResponse(valid=False, errors=[str(e)])
