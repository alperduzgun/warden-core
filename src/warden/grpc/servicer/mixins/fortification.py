...
class FortificationMixin:
    ...
    async def GetFortificationSuggestions(self, request, context) -> "warden_pb2.FortificationResult":
        ...
        try:
            ...
            if hasattr(self.bridge, "get_fortification_suggestions"):
                result = await self.bridge.get_fortification_suggestions(
                    path=request.path, fortifiers=list(request.fortifiers) if request.fortifiers else None
                )
                ...
                for suggestion in result.get("suggestions", []):
                    response.suggestions.append(ProtoConverters.convert_fortification_suggestion(suggestion))

                return response

            return warden_pb2.FortificationResult(success=False, error_message="Fortification analysis not available")

        except Exception as e:
            logger.error("grpc_get_fortification_error: %s", str(e))
            return warden_pb2.FortificationResult(success=False, error_message=str(e))

    async def ApplyFortification(self, request, context) -> "warden_pb2.ApplyFortificationResponse":
        ...
        try:
            if hasattr(self.bridge, "apply_fortification"):
                result = await self.bridge.apply_fortification(
                    suggestion_id=request.suggestion_id, dry_run=request.dry_run
                )
            ...
            return warden_pb2.ApplyFortificationResponse(
                success=False, error_message="Fortification application not available"
            )

        except Exception as e:
            logger.error("grpc_apply_fortification_error: %s", str(e))
            return warden_pb2.ApplyFortificationResponse(success=False, error_message=str(e))
    ...
    async def GetSecurityScore(self, request, context) -> "warden_pb2.SecurityScoreResponse":
        ...
        try:
            ...
            if not issues:
                ...
                security_issues = [i for i in issues if i.get("frame_id") == "security"]
                weighted = sum(
                    10
            ...
            response = warden_pb2.SecurityScoreResponse(overall_score=score, grade=grade)
            ...
            return warden_pb2.SecurityScoreResponse(overall_score=0.0, grade="F")
