...
class DuplicationAnalyzer(BaseCleanupAnalyzer):
    """
    ...
    async def analyze_async(
        ...
        Returns:
            CleanupResult with duplication issues
        """
    ...
    def _analyze_duplication(self, code: str) -> List[CleanupIssue]:
        """
        Analyze code for duplication.
        ...
        for block1_start, block2_start, length in duplicate_blocks:
            issues.append(
                CleanupIssue(
                    issue_type=CleanupIssueType.CODE_DUPLICATION,
                    description=f"Duplicate code block of {length} lines (also at line {block2_start})",
                    line_number=block1_start,
                    severity=CleanupIssueSeverity.HIGH if length > 5 else CleanupIssueSeverity.MEDIUM,
                )
            )
