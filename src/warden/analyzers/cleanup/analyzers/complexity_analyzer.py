...
class ComplexityAnalyzer(BaseCleanupAnalyzer):
    """
    ...
    async def analyze_async(
        ...
        Returns:
            CleanupResult with complexity issues
        """
        ...
        try:
            ...
            return CleanupResult(
                ...
                analyzer_name=self.name,
                metrics={
                    "long_methods": sum(1 for i in issues if i.issue_type == CleanupIssueType.LONG_METHOD),
                    "complex_methods": sum(1 for i in issues if i.issue_type == CleanupIssueType.COMPLEX_METHOD),
                },
            )
    ...
    def _analyze_complexity(self, code: str) -> List[CleanupIssue]:
        """
    ...
    def _check_function_complexity(
        ...
        if func_lines > MAX_FUNCTION_LINES:
            issues.append(
                CleanupIssue(
                    issue_type=CleanupIssueType.LONG_METHOD,
        ...
        if param_count > MAX_PARAMETERS:
            issues.append(
                CleanupIssue(
                    issue_type=CleanupIssueType.COMPLEX_METHOD,
    ...
    def _create_suggestion(self, issue: CleanupIssue, code: str) -> CleanupSuggestion:
        ...
        if issue.issue_type == CleanupIssueType.LONG_METHOD:
