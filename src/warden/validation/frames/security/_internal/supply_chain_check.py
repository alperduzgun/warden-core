...
POPULAR_PYTHON_PACKAGES: set[str] = {
    # Web frameworks
    "django", "flask", "fastapi", "tornado", "bottle", "pyramid", "starlette",
    "sanic", "falcon", "quart", "aiohttp", "uvicorn", "gunicorn", "hypercorn",
    # Data science / ML
    "numpy", "pandas", "scipy", "matplotlib", "scikit-learn", "sklearn",
    "tensorflow", "torch", "keras", "xgboost", "lightgbm", "catboost",
    "seaborn", "plotly", "bokeh", "statsmodels", "nltk", "spacy",
    "transformers", "datasets", "tokenizers", "accelerate", "diffusers",
    "opencv-python", "pillow", "imageio",
    # HTTP / Networking
    "requests", "httpx", "urllib3", "aiohttp", "httplib2", "httpcore",
    "websockets", "grpcio", "paramiko",
    # Database
    "sqlalchemy", "psycopg2", "pymongo", "redis", "celery", "alembic",
    "pymysql", "asyncpg", "motor", "peewee", "tortoise-orm", "databases",
    # Cloud / DevOps
    "boto3", "botocore", "google-cloud-storage", "azure-storage-blob",
    "docker", "kubernetes", "ansible", "fabric", "paramiko",
    # CLI / Utils
    "click", "typer", "rich", "tqdm", "colorama", "pyyaml", "toml",
    "python-dotenv", "pydantic", "attrs", "dataclasses-json",
    "marshmallow", "cerberus",
    # Testing
    "pytest", "unittest2", "coverage", "tox", "nox", "hypothesis",
    "factory-boy", "faker", "responses", "httpretty", "vcrpy",
    "pytest-asyncio", "pytest-cov", "pytest-mock",
    # Security / Crypto
    "cryptography", "pyjwt", "bcrypt", "passlib", "certifi", "pyopenssl",
    # Serialization / Parsing
    "lxml", "beautifulsoup4", "html5lib", "feedparser", "xmltodict",
    ...
    "bandit", "pyright",
    # Misc popular
    "jinja2", "markupsafe", "werkzeug", "itsdangerous", "six",
    "setuptools", "pip", "wheel", "twine", "build",
    "packaging", "importlib-metadata", "typing-extensions",
    "python-dateutil", "arrow", "pendulum", "pytz",
    "chardet", "charset-normalizer", "idna",
    "filelock", "watchdog", "schedule", "apscheduler",
}

POPULAR_JS_PACKAGES: set[str] = {
    # Frameworks
    "react", "vue", "angular", "svelte", "next", "nuxt", "gatsby",
    "remix", "astro", "solid-js", "preact", "lit",
    # Runtime / Build
    ...
    "puppeteer", "testing-library", "sinon", "nyc",
    # HTTP / Networking
    "axios", "node-fetch", "got", "superagent", "undici",
    "socket.io", "ws",
    # Database / ORM
    "mongoose", "sequelize", "knex", "prisma", "typeorm", "drizzle-orm",
    ...
    "uuid", "nanoid", "chalk", "debug", "dotenv",
    "commander", "yargs", "inquirer", "ora", "glob",
    "fs-extra", "rimraf", "mkdirp", "cross-env",
    "semver", "minimist", "yaml",
    # Validation / Schema
    ...
    # Misc popular
    "rxjs", "graphql", "apollo-server", "cors",
    "body-parser", "cookie-parser", "multer", "morgan",
    "helmet", "compression", "serve-static",
    "concurrently", "nodemon", "pm2", "ts-node",
    "tsup", "tsconfig-paths",
}
...
def check_typosquatting(
    ...
    popular_normalized: dict[str, str] = {
        _normalize_package_name(pkg): pkg for pkg in popular_packages
    }
