"""
init_steps.py — Extracted step functions for warden init.

Each function corresponds to a logical phase of the init_command orchestrator.
They are pure mechanical extractions: no logic changes, only decomposition.
"""
...
# ---------------------------------------------------------------------------
# MetaAdapter — thin wrapper mapping ProjectContext to the shape init expects
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Private scaffold helpers (moved from init.py)
# ---------------------------------------------------------------------------


def _generate_ignore_file(root: Path, meta) -> None:
...
def _setup_semantic_search(config_path: Path) -> None:
    """
...
async def _create_baseline_async(root: Path, config_path: Path) -> None:
...
async def _generate_intelligence_async(root: Path, config_files: dict | None = None) -> None:
    """
...
def detect_project(warden_dir: Path) -> tuple["MetaAdapter", object]:
    """Detect project environment using ProjectStructureAnalyzer.

    Returns (meta, context) where meta is a MetaAdapter and context is the
    raw ProjectContext from the analyzer.
    """
    ...
    console.print(
        f"   [green]✓[/green] Framework: [cyan]{', '.join(meta.frameworks) if meta.frameworks else 'None detected'}[/cyan]"
    )
...
def select_mode(mode: str, is_interactive: bool, meta: "MetaAdapter") -> tuple[str, dict]:
    ...
    return mode_choice, mode_config


# ---------------------------------------------------------------------------
# Step 3: LLM configuration and .env updates
# ---------------------------------------------------------------------------


def configure_llm_and_env(
    provider: str | None,
    existing_config: dict,
    is_interactive: bool,
) -> tuple[dict, str, str]:
    """Configure LLM provider and update .env / .env.example.

    Returns (llm_config, provider, model).
    """
    ...
    if new_env_vars:
        ...
        def ensure_line(key: str) -> None:
            nonlocal example_lines
            if not any(ln.split("=")[0] == key for ln in example_lines if "=" in ln):
    ...
    return llm_config, provider, model


# ---------------------------------------------------------------------------
# Step 5: Config generation / merge
# ---------------------------------------------------------------------------
...
def generate_config(
    ...
    """Generate .warden/config.yaml (new) or merge into existing one.
...
# ---------------------------------------------------------------------------
# Step 5.5 – 8: Scaffold generation
# ---------------------------------------------------------------------------


def generate_scaffolds(warden_dir: Path, config_path: Path, meta: "MetaAdapter") -> None:
    """Generate taint catalog, tech debt tracker, ignore files, rules, and config comments."""
...
"""
    ...
    # --- Step 8b: Additional scaffolds (security rules, suppressions, context, .gitignore) ---
    _generate_extra_scaffolds(warden_dir, meta)


def _generate_extra_scaffolds(warden_dir: Path, meta: "MetaAdapter") -> None:
    """Generate additional scaffold files that were previously missing from init.

    Only creates files that don't already exist (safe for re-init).
    """
    rules_dir = warden_dir / "rules"
    rules_dir.mkdir(exist_ok=True)
    ...
    # --- Security Rules ---
    _write_if_missing(
        rules_dir / "security.yaml",
        "security rules",
        """\
# Security rules — detects hardcoded secrets, API keys, and common vulnerabilities
# Auto-generated by warden init
...
rules:
  - id: env-var-api-keys
    ...
    severity: critical
    isBlocker: true
    description: API keys must be loaded from environment variables, never hardcoded
    enabled: true
    type: security
    conditions:
      secrets:
    ...
    message: API keys must use environment variables. Never hardcode secrets!
    exceptions:
      - "*/test_*.py"
      - "**/tests/**"
      - ".env.example"
  ...
  - id: no-secrets
    ...
    severity: critical
    isBlocker: true
    description: Detects passwords, tokens, and private keys that should never be committed
    enabled: true
    type: security
    conditions:
      secrets:
    ...
    message: Secrets detected. Use environment variables or secret management!
    exceptions:
      - "*/test_*.py"
      - "**/tests/**"
      - ".env.example"
""",
    ...
    # --- Convention Rules ---
    _write_if_missing(
        rules_dir / "conventions.yaml",
        "convention rules",
        """\
# Code convention rules — naming, logging, debugging
# Auto-generated by warden init
...
rules:
  - id: no-print
    ...
    conditions:
      patterns:
        - "print\\\\("
    message: Use logger.info/debug instead of print()
    exceptions:
      - "*/test_*.py"
      - "**/tests/**"
      - "scripts/**"
...
""",
    ...
    _write_if_missing(
        warden_dir / "suppressions.yaml",
        "suppressions template",
        """\
# Warden Suppression Configuration
# Manage false positive suppressions here.
#
# Example:
#   entries:
#     - id: "my-suppression"
...
""",
    ...
    # --- Context Template ---
    _write_if_missing(
        warden_dir / "context.yaml",
        "project context",
        f"""\
# Project context — helps Warden understand your codebase conventions
# Auto-generated by warden init, customize as needed
...
""",
    ...
    # --- .gitignore ---
    _write_if_missing(
        warden_dir / ".gitignore",
        ".gitignore",
        """\
# Warden generated/cached files — do not commit
cache/
embeddings/
reports/
intelligence/
...
""",
    ...
    frames_dir = warden_dir / "frames"
    example_frame_dir = frames_dir / "example-custom"
    if not example_frame_dir.exists():
        example_frame_dir.mkdir(parents=True, exist_ok=True)
        _write_if_missing(
            example_frame_dir / "frame.yaml",
            "example custom frame",
            """\
# Example Custom Frame Definition
# Copy this directory and modify to create your own frame.
#
# A frame needs:
#   1. frame.yaml  — metadata and config schema
#   2. frame.py    — Python class extending ValidationFrame
#
# To activate: add the frame id to config.yaml `frames:` list

name: "Example Custom Frame"
id: "example-custom"
version: "1.0.0"
author: "Your Name"
description: "Template for creating custom validation frames"
...
min_warden_version: "2.0.0"

config_schema:
  enabled:
    type: "boolean"
    default: false
    description: "Enable this frame (set to true when ready)"
...
""",
        ...
        console.print(f"[green]Created example custom frame: [bold]{example_frame_dir}[/bold][/green]")


def _write_if_missing(path: Path, label: str, content: str) -> None:
    """Write a file only if it doesn't already exist."""
    if path.exists():
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(content, encoding="utf-8")
    console.print(f"[green]Created {label}: [bold]{path}[/bold][/green]")


# ---------------------------------------------------------------------------
# Steps 9 – 16: Post-setup (semantic search, agent, baseline, intel, CI, grammars, context)
# ---------------------------------------------------------------------------
...
def run_post_setup(
    ...
    """Run all post-setup steps: semantic index, agent/MCP, baseline, intelligence, CI, frames, grammars, context."""
    ...
    try:
        ...
        console.print(
            f"[green]✓ Found {len(frames)} built-in frames: {', '.join(frame_names[:5])}{'...' if len(frames) > 5 else ''}[/green]"
        )
    except Exception as e:
    ...
    try:
        ...
        if not detected_code_langs:
            ...
            if not missing_grammars:
                console.print(
                    f"[green]✓ All grammars already installed for: {', '.join([l.value for l in detected_code_langs])}[/green]"
                )
            else:
