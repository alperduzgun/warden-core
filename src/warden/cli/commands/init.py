...
from warden.cli.commands.init_helpers import configure_llm, configure_vector_db, configure_agent_tools
...
def _generate_ignore_file(root: Path, meta):
    """Generate .wardenignore based on project type and template."""
    ...
    try:
        try:
             raw_yaml = importlib.resources.files("warden.templates").joinpath("ignore.yaml").read_text(encoding="utf-8")
        except AttributeError:
             raw_yaml = importlib.resources.read_text("warden.templates", "ignore.yaml", encoding="utf-8")
        
        ignore_cfg = yaml.safe_load(raw_yaml)
        
        content.append("# Warden Ignore File")
        content.append("# Auto-generated by Smart Init")
        content.append("")
        content.append("# --- Default Patterns (from template) ---")
        
        for d in ignore_cfg.get("directories", []):
            content.append(f"{d}/")
        
        for f in ignore_cfg.get("file_patterns", []):
            content.append(f"{f}")
            
        for p in ignore_cfg.get("path_patterns", []):
            content.append(f"{p}")
            
    except Exception as e:
        console.print(f"[yellow]Warning: Could not load ignore template: {e}[/yellow]")
        content.extend([
            "# Warden Ignore File (Fallback)",
            ".git/", "node_modules/", "venv/", ".venv/", "__pycache__/", "*.log"
        ])

    content.append("")
    content.append("# --- Language Specific ---")
...
def _run_preflight_check() -> bool:
    """Run a quick dependency check before initialization."""
    from warden.services.package_manager.doctor import WardenDoctor, CheckStatus
    
    # We pass current dir, but dependencies check doesn't need valid project
    doc = WardenDoctor(Path.cwd())
    status, msg = doc.check_dependencies()
    
    if status == CheckStatus.ERROR:
        console.print("[bold red]⛔ Pre-flight Check Failed[/bold red]")
        console.print(f"[red]Error: {msg}[/red]")
        console.print("[yellow]Please install missing dependencies and try again.[/yellow]")
        return False
...
def init_command(
    ...
    """
    # Quick pre-flight check for critical dependencies
    ...
    # 1. Base Core Frames
    meta.suggested_frames = ["security", "resilience", "orphan", "spec", "gitchanges"]
    ...
    if new_env_vars:
        ...
        if not env_example_path.exists():
            with open(env_example_path, "w") as f:
                ...
                # We need provider info here, but loop logic below defines it. 
                # Let's extract it from llm_config now.
                ex_provider = llm_config.get("provider", "unknown")
                
                if ex_provider == "azure":
                    f.write("AZURE_OPENAI_API_KEY=\n")
                    f.write("AZURE_OPENAI_ENDPOINT=\n")
                    f.write("AZURE_OPENAI_DEPLOYMENT_NAME=\n")
                else:
                     # Generic key
                     f.write(f"{ex_provider.upper()}_API_KEY=\n")
    ...
    if not config_path.exists():
        ...
        try:
            try:
                raw_config = importlib.resources.files("warden.templates").joinpath("config.yaml").read_text(encoding="utf-8")
            except AttributeError:
                raw_config = importlib.resources.read_text("warden.templates", "config.yaml", encoding="utf-8")
            config_data = yaml.safe_load(raw_config)
        except Exception as e:
            console.print(f"[yellow]Warning: Could not load config template: {e}[/yellow]")
            config_data = {"version": "1.0.0", "project": {}, "llm": {}, "frames": [], "frames_config": {}, "settings": {}, "semantic_search": {}}

        # Overlay Project Info
        config_data["project"]["name"] = Path.cwd().name
        config_data["project"]["language"] = meta.language
        config_data["project"]["type"] = meta.project_type
        config_data["project"]["frameworks"] = meta.frameworks
        ...
        if "fast_model" in llm_config:
             config_data["llm"]["fast_model"] = llm_config["fast_model"]
        config_data["llm"]["timeout"] = 300
        
        # Overlay Frames
        config_data["frames"] = meta.suggested_frames
        
        # Build frames_config based on detection
        if "frames_config" not in config_data: config_data["frames_config"] = {}
        for frame in meta.suggested_frames:
            if frame not in config_data["frames_config"]:
                config_data["frames_config"][frame] = {"enabled": True}
        
        # Apply mode settings
        if mode_choice == "1": # Vibe
            for f in config_data["frames_config"]:
                 config_data["frames_config"][f]["severity_threshold"] = "critical"
        
        # Settings
        if "settings" not in config_data: config_data["settings"] = {}
        config_data["settings"]["fail_fast"] = mode_config.get("fail_fast", False)
        config_data["settings"]["enable_classification"] = True
        config_data["settings"]["mode"] = ["vibe", "normal", "strict"][int(mode_choice)-1]
        config_data["settings"]["use_llm"] = provider != "none"
        config_data["settings"]["use_local_llm"] = llm_config.get("use_local_llm", False)
        
        # Semantic Search
        config_data["semantic_search"] = vector_config
